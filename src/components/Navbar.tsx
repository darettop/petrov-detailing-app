  ...
  return (
    <>
      <nav className={cn(
        ...
        {/* Mobile Menu Trigger */}
        <button 
          className="md:hidden text-gold p-2 z-50"
      ...
      {/* Mobile Menu Overlay - Outside nav to avoid filter containing block bug */}
      <AnimatePresence>
        {mobileMenuOpen && (
          <motion.div
            ...
            className="fixed inset-0 z-[100] bg-bg-black/98 backdrop-blur-3xl flex flex-col items-center justify-center gap-12"
          >
            <button 
            ...
            <Link 
              href="#book" 
              onClick={() => setMobileMenuOpen(false)}
              className="mt-4 bg-gold text-bg-black px-10 py-4 text-sm font-bold uppercase tracking-widest hover:bg-gold-hover transition-colors"
            >
              Book Now
            </Link>
          </motion.div>
